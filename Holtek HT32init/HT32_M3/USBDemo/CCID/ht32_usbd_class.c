/*********************************************************************************************************//**
 * @file    USBD/HID_Demo/ht32_usbd_class.c
 * @version $Rev:: 2393         $
...
static void USBDClass_EPT1OUTProcess(void);
static void USBDClass_EPT2INProcess(void);
static void USBDClass_EPT3INProcess(void);
...
static vu32 gOutEPLen = 0;


__ALIGN4 static u8 gOutputBuffer[64];
__ALIGN4 static u8 gInputBuffer[64];
__ALIGN4 static u8 gInputBuffer2[64];
...
static void USBDClass_MainRoutine(u32 uPara)
{
  USBDClass_EPT1OUTProcess();
  USBDClass_EPT2INProcess();
  USBDClass_EPT3INProcess();
}



/*********************************************************************************************************//**
  * @brief  USB Class Endpoint 1 OUT Process for application.
  * @retval None
  ***********************************************************************************************************/
static void USBDClass_EPT1OUTProcess(void)
{
  if (gIsOUTReady == TRUE)
  {
    gOutEPLen = USBDCore_EPTReadOUTData(USBD_EPT1, (u32 *)gOutputBuffer, 64);
...
/*********************************************************************************************************//**
  * @brief  USB Class Endpoint 2 IN Process for application.
  * @retval None
  ***********************************************************************************************************/
static void USBDClass_EPT2INProcess(void)
{
  ...
  if (gIsReadyToLoopBack == TRUE)
  {
    ...
    {
      gIsINEmpty = FALSE;
      gIsReadyToLoopBack = FALSE;
      for (i = gOutEPLen - 1; i >= 0; i--)
      {
        gInputBuffer[i] = gOutputBuffer[i];
      }
      len = USBDCore_EPTGetBufferLen(USBD_EPT2);
      USBDCore_EPTWriteINData(USBD_EPT2, (u32 *)gInputBuffer, len);
    }
  }
}

/*********************************************************************************************************//**
  * @brief  USB Class Endpoint 3 IN Process for application.
  * @retval None
  ***********************************************************************************************************/
static void USBDClass_EPT3INProcess(void)
{
  if (gIsEP3INEmpty == TRUE)
  {
    gIsEP3INEmpty = FALSE;
    USBDCore_EPTWriteINData(USBD_EPT3, (u32 *)gInputBuffer2, 4);
  }
}
...
static void USBDClass_Request(USBDCore_Device_TypeDef *pDev)
{
  ...
  switch (uUSBCmd)
  {
    ...
    /*------------------------------------------------------------------------------------------------------*/
    /* | 01_xxxxxxxxxx        | 80_Device-to-Host       | 20_Class Request    | 1_Interface | xxxxh         */
    /*------------------------------------------------------------------------------------------------------*/
    case (0x0000):
    {
      //USBDClass_xxxx(pDev);
