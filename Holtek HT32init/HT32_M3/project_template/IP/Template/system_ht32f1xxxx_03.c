/**************************************************************************//**
 * @file    library/Device/Holtek/HT32F1xxxx/Source/system_ht32f1xxxx_03.c
...
//        <o7>  PLL Feedback Clock Divider (NF2): 1 ~ 32
//                <1-32:1>
//                <i> PLL feedback clock = PLL clock source x NF2
//                <i> PLL feedback clock must be in the range of 64 MHz to 80 MHz
...
//                <i> PLL output clock must be in the range of 8 MHz to 80 MHz
//    </e>
//
 ...
 */
  ...
  #define VCO_MAX 80000000UL
  #define PLL_MIN  8000000UL
  #define PLL_MAX 80000000UL
...
#define CKAHB_MAX 80000000UL
#define WS0_CLK   20000000UL
#define WS1_CLK   40000000UL
#define WS2_CLK   60000000UL
...
void SystemInit(void)
{
  ...
  do {
    HT_PWRCU->CR = (HT_PWRCU->CR & ~(3UL << 4)) | (3UL << 4);
  } while (((HT_PWRCU->CR >> 4) & 3UL) != 3UL);
#else
  do {
...
void SystemCoreClockUpdate(void)
{
  ...
  u32 PllFeedbackClockDiv = (((HT_CKCU->PLLCFGR >> 23) & 31UL) == 0) ? (32) : ((HT_CKCU->PLLCFGR >> 23) & 31UL);
