...
u32 SERIAL_PutChar(u32 ch)
{
  ...
  /* Wait until user open the virtual COM port by PC UI such as Hyper Terminal                              */
  while (uIsTerminalOpened != 2);
  #elif (RETARGET_USB_MODE == RETARGET_USB_MODE_NONBLOCK)
  /* Drop data if USB is not ready                                                                          */
  if (USBDCore_GetStatus() != USB_STATE_CONFIGURED)
  {
    return ch;
  }
  #endif
  ...
  TxBuffer[TxCount++] = ch;
  #if (RETARGET_TXBUFFER_SIZE > 1)
    #warning Please use "SERIAL_Flush()" to sent out the buffer data immediately.
  #endif
...
void SERIAL_USBDClass_RXHandler(USBD_EPTn_Enum EPTn)
{
  ...
  uLen = USBDCore_EPTReadOUTData(EPTn, (u32*)uTempBuffer, uLen);
